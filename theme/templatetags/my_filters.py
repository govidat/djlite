...
def get_dictid(dict_array=[], arg=0):
    ...
    """ To be deprecated and replaced with a combination of get_dict_filtered_by_id; get_dict_by_client_and_prioritized_values
...
@register.filter
def get_list_filtered_by_token(value=[], arg=''):
    # filtered_data = list(filter(lambda item: not item.get('is_active'), data))
    return list(filter(lambda item: item.get('token') == arg, value))

@register.filter
def get_dict_by_client_and_prioritized_values(list_of_dicts=[], keyvals=""):
    """
    Retrieves a dictionary from a list of dictionaries based on client
    and a prioritized list of values to check for that key.

    Args:
        list_of_dicts: A list of dictionaries to search within.
        'client': The key to check for in each dictionary.
        keyvals: A comma separated string of values to check against the client value, in order of priority.

    Returns:
        The first dictionary found where the value associated with the key='client'
    ...
    return None
    """ Test Data
    source_list = [{'client': 'store_porur', 'title': 'Saravana Porur'}, {'client': 'store_ho', 'title': 'Saravana HO'}, {'client': 'default', 'title': 'Defaut Title'} ]
    keyvals = 'store_porur,store_ho'
    result = get_dict_by_client_and_prioritized_values(source_list, keyvals)
    print(result)
    """

@register.filter
def get_listdict_by_token(listdict=[], arg=''):
    return list(filter(lambda x: x['token'] == arg, listdict))
