...
from mysite.models import ClientFeatureControl   # adjust import path if needed

_DEMAND_FEATURES = frozenset([
    "demand_planning",
    "actuals_upload",
    "forecast_run",
    "consensus_override",
...
def feature_is_enabled(client, feature: str) -> bool:
    """
    Return True if the feature is NOT currently disabled for this client.

    Evaluation order (mirrors ClientFeatureControl.is_active semantics):
      1. If a client-specific row exists and is active right now → disabled.
      2. If a global row (client=NULL) exists and is active right now → disabled.
      3. Otherwise → enabled.

    Usage in a view:
        if not feature_is_enabled(request.client, "demand_planning"):
            raise PermissionDenied

    Usage in a Celery task:
        if not feature_is_enabled(client, "forecast_run"):
            return {"status": "skipped", "reason": "feature disabled"}
    """
    assert feature in _DEMAND_FEATURES, f"Unknown demand feature: {feature!r}"

    now = timezone.now()
    blocked = ClientFeatureControl.objects.filter(
        feature=feature,
        is_disabled=True,
        from_date__lte=now,
        to_date__gte=now,
    ).filter(
        # client-specific OR global (client=NULL)
        client__in=[client] if client else []
    ).union(
        ClientFeatureControl.objects.filter(
            feature=feature,
            is_disabled=True,
            from_date__lte=now,
            to_date__gte=now,
...
def get_disabled_message(client, feature: str) -> str:
    """
    Return the message string from the active blocking row, if any.
    ...
    row = (
        ...
        .filter(client__in=[client] if client else [])
        .first()
        or ClientFeatureControl.objects.filter(
            feature=feature,
            is_disabled=True,
            from_date__lte=now,
            to_date__gte=now,
    ...
    if row:
        return row.message or "This feature is temporarily unavailable."
    return ""
```

Note that `feature_is_enabled` intentionally mirrors the `is_active` property on `ClientFeatureControl` (the `is_disabled AND from_date ≤ now ≤ to_date` logic) but queries at the database level rather than loading all rows into Python — important when called inside a request/response cycle.
...
- [ ] `ClientFeatureControl` `FEATURE_CHOICES` updated and `AlterField` migration applied cleanly
