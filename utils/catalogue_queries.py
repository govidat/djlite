...
def get_filterable_attribute_types(client, taxonomy_slug, active_node_ids=None):
    ...
    # Only scope to selected nodes if nodes are actually selected
    if active_node_ids:
    ...
    # else: no filter — return attr types from all nodes in this taxonomy
...
def get_item_queryset(client, filters=None):
    ...
    qs = Item.objects.filter(
        ...
        Prefetch(
            'medias',
            queryset=ItemMedia.objects.filter(is_primary=True).order_by('order'),
            to_attr='prefetched_primary_medias'
        ),
        Prefetch(
            'taxonomy_mappings',
...
def _expand_node_ids_grouped(node_id_list):
    ...
    #print(f"DEBUG taxonomy groups: {[(k, [n.slug for n in v]) for k,v in by_tax.items()]}")
    ...
    #print(f"DEBUG expanded groups: {[list(g) for g in groups]}")
    return groups
...
def serialize_item(item):
    ...
    primary_medias = getattr(item, 'prefetched_primary_medias', [])
...
def build_catalogue_payload(client, filters=None, page_number=1, per_page=24):
    ...
    for taxonomy in resolved_taxonomies:
        ...
        #    )
        # With — always load attr types for the whole taxonomy:
        active_node_ids = filters.get('node_ids', [])
        attr_type_qs = get_filterable_attribute_types(
            client, taxonomy.slug,
            active_node_ids if active_node_ids else None  # None = all nodes
        )            
        attr_types = [
            {
                'id':         at.id,
